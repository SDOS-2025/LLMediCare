...
class MedicationFactory(DjangoModelFactory):
    ...
    user = factory.SubFactory(UserFactory)
    name = factory.Faker('word')
    dosage = factory.LazyAttribute(lambda x: f"{random.randint(10, 500)}mg")  # Updated to random.randint
    frequency = factory.LazyAttribute(lambda x: f"{random.randint(1, 3)} times daily")  # Updated to random.randint
    start_date = factory.LazyFunction(timezone.now().date)
    end_date = factory.LazyAttribute(lambda x: timezone.now().date() + timedelta(days=random.randint(7, 30)))  # Updated to random.randint
    instructions = factory.Faker('text', max_nb_chars=100)
...
class AppointmentFactory(DjangoModelFactory):
    ...
    patient = factory.SubFactory(UserFactory, role='patient')
    doctor = factory.SubFactory(UserFactory, role='doctor')
    appointment_date = factory.LazyAttribute(lambda x: timezone.now().date() + timedelta(days=random.randint(1, 14)))  # Updated to random.randint
    start_time = factory.LazyAttribute(lambda x: f"{random.randint(8, 16)}:00:00")  # Updated to random.randint
    end_time = factory.LazyAttribute(lambda x: f"{random.randint(9, 17)}:00:00")  # Updated to random.randint
    notes = factory.Faker('text', max_nb_chars=100)
    status = 'pending'
